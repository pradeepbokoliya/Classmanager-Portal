...
def StudentSignUp(request):
    ...
    if request.method == "POST":
        ...
        if user_form.is_valid() and student_profile_form.is_valid():
            ...
            # Generate and assign roll number from model helper
            try:
                profile.roll_no = Student.generate_roll_no()
            except Exception:
                # Fallback: simple incremental numeric if generation fails
                existing = Student.objects.count()
                profile.roll_no = f"STU{existing+1:04d}"
            profile.user = user
            profile.save()
